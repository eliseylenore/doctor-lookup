  ...
  if (response.data[0].ratings.length > 0) {
    if (response.data[0].ratings[0] !== undefined) {
      if(response.data[0].ratings[0].image_url_small !== undefined) {
        console.log(response.data[0].ratings[0].image_url_small);
      }
      else {
        console.log(response.data[0].ratings[1].image_url_small)
      }
      console.log("rating is not undefined at ratings[0]");
    }
  }
  else {
    console.log("doesn't have rating");
  }
   })
  .fail(function(error){
     console.log("fail");
